...
const bIsInIframe = iframeCheck();

let MAX_FPS = 30;
let MIN_BITRATE = 100;
let MAX_BITRATE = 1000;
let MIN_QP = 0;
let MAX_QP = 15;

let noSleep = new NoSleep();
let bNoSleepEnabled = false;
...
let bClientPlatformEmitted = false;
...
let bFirstMouseOrTouchMove = true;
...
const bIsRunningOnMobile = mobileAndTabletCheck();

const scrollKeys = [32, 33, 34, 35, 37, 38, 39, 40];
const genericPreventDefaultEvents = ["scroll", "wheel", "DOMMouseScroll", "touchmove"];
const arrowPreventDefaultEvents = ["keyup", "keydown", "keypress"];
let genericPreventDefault = function (e) {
    console.log("genericPreventDefault");
    e.preventDefault();
};
let arrowKeysHandler = function (e) {
    if (scrollKeys.includes(e.keyCode)) {
        genericPreventDefault(e);
    }
};
window.onmessage = function (e) {
    if (e.data == "disallowDefault") {
        genericPreventDefaultEvents.forEach(evName => document.addEventListener(evName, genericPreventDefault, { passive: false }));
        arrowPreventDefaultEvents.forEach(evName => document.addEventListener(evName, arrowKeysHandler, { passive: false }));
    }
    else if (e.data == "allowDefault") {
...
// Common codes between C# and JS
let WebSocketCloseStatus =
{
	NormalClosure: 1000,
    //Rest is unavailable to be send from client to server.
...
function setupHtmlEvents() {
    ...
    if (!bIsRunningOnMobile) {
        qualityStatus.addEventListener('click', onExpandOverlay_Click);
        fullscreenButton.addEventListener('click', onFullscreen_Click);

        document.addEventListener('fullscreenchange', onFullScreen_Change);
        document.addEventListener("fullscreenchange", onFullScreen_Change);
        document.addEventListener("mozfullscreenchange", onFullScreen_Change);
        document.addEventListener("webkitfullscreenchange", onFullScreen_Change);
        document.addEventListener("msfullscreenchange", onFullScreen_Change);
    }
    else {
...
function showConnectOverlay() {
    ...
    startText.innerHTML = 'Click to start interaction';
...
function setupStats(){
    ...
    webRtcPlayerObj.onAggregatedStats = (aggregatedStats) => {
        ...
        let qualityStatusImg = document.getElementById("qualityStatusImg");
        ...
        let blinkQualityStatus = function(speed) {
            ...
            let tickId = setInterval(
                function() {
                    ...
                    qualityStatusImg.style = `opacity: ${Math.abs((opacity - 0.5) * 2)}`;
                    if (opacity <= 0.1) {
                        if (--iter == 0) {
        ...
        qualityStatusImg.src = "images/network_"+color+".png";
...
function updateVideoStreamSize() {
    ...
    if (now - lastTimeResized > 1000) {
        ...
        emitCommand({ ConsoleCommand: 'PixelStreaming.Encoder.MinQP ' + MIN_QP });
        emitCommand({ ConsoleCommand: 'PixelStreaming.Encoder.MaxQP ' + MAX_QP });
        emitCommand({ ConsoleCommand: 'PixelStreaming.WebRTC.Fps ' + MAX_FPS });
        emitCommand({ ConsoleCommand: 'PixelStreaming.WebRTC.MinBitrate ' + MIN_BITRATE * 1000 });
        emitCommand({ ConsoleCommand: 'PixelStreaming.WebRTC.MaxBitrate ' + MAX_BITRATE * 1000 });
...
function emitMouseMove(x, y, deltaX, deltaY) {
    if (bFirstMouseOrTouchMove) {
        bFirstMouseOrTouchMove = false;
        emitUIInteraction({
            playerFirstMouseInteraction: true
        });
    }
...
function registerLockedMouseEvents(playerElement) {
    ...
    function lockStateChange() {
        if (document.pointerLockElement === playerElement ||
            ...
            // In case the app is running under an iframe
            if (bIsInIframe) {
                window.top.postMessage("desktopPointerLocked", "*");
            }
        } else {
            ...
            // In case the app is running under an iframe
            if (bIsInIframe) {
                window.top.postMessage("desktopPointerUnlocked", "*");
            }
        }
    }
...
function onFullScreen_Change( /* e */) {
    let overlay = document.getElementById('overlay');
    let fullscreenImg = document.getElementById("fullscreenImg");

    if (!bFullscreen) {
        if (!overlay.classList.contains("fullscreen-enabled")) {
            overlay.classList.add("fullscreen-enabled");
        }
        fullscreenImg.src = "images/fullscreen_lime.png";

        // In case the app is running under an iframe
        if (!bIsRunningOnMobile && bIsInIframe) {
            window.top.postMessage("desktopFullscreenEnabled", "*");
        }
    }
    else {
        if (overlay.classList.contains("fullscreen-enabled")) {
            overlay.classList.remove("fullscreen-enabled");
        }
        fullscreenImg.src = "images/fullscreen_gray.png";

        // In case the app is running under an iframe
        if (!bIsRunningOnMobile && bIsInIframe) {
            window.top.postMessage("desktopFullscreenDisabled", "*");
        }
    }
    bFullscreen = !bFullscreen;
}

function onFullscreen_Click( /* e */) {
    if (!bIsRunningOnMobile) {
        if (!bFullscreen) {
    ...
    else if (bIsInIframe) /*on mobile and in iframe*/{
        onFullScreen_Change(); //Call manually for visual changes to the fullscreen button
        if (bFullscreen) {
            window.top.postMessage("mobileFullscreenEnabled", "*");
        }
        else {
            window.top.postMessage("mobileFullscreenDisabled", "*");
        }
    }
}
...
function start() {
    ...
    let qualityStatusImg = document.getElementById("qualityStatusImg");
    if (qualityStatusImg) {
        qualityStatusImg.src = "images/network_gray.png";
    }
...
function showConnectingPleaseWaitOverlay() {
    let textOverlay = document.createElement('div');
    textOverlay.id = 'connectingPleaseWaitOverlay';
    textOverlay.innerHTML = 'Connecting... Please wait.';
    setOverlay('textDisplayState', textOverlay);
}

let bWaitingForFirstMessageFromWSServer = true;
...
function connect() {
    ...
    // In case the app is running under an iframe
    if (bIsInIframe) {
        window.top.postMessage("loadingPhaseStarted", "*");
    }

    if (!bNoSleepEnabled) {
        bNoSleepEnabled = true;
        try { noSleep.enable().catch(function () { }); } catch (err) { }
    }
    ...
    showConnectingPleaseWaitOverlay();
    ...
    ws.onmessage = function(event) {
        ...
        {
            let infoFromOrchestrator = event.data.substring('--INFO--'.length);
            let overlayElement = document.getElementById('connectingPleaseWaitOverlay');
            if (overlayElement !== null) {
                overlayElement.innerHTML = infoFromOrchestrator + '<br>Please wait...';
            }
            console.log('Info from orchestrator: ' + infoFromOrchestrator);
            return;
        }
        ...
        try {
            ...
            console.error(`cannot parse the message from server: ${event.data}\nError: ${err}`);
            ws.close(WebSocketCloseStatus.NormalClosure, 'Cannot parse the data received from the server.');
            return;
        }
        if (msg.type === 'config') {
        ...
        if (bWaitingForFirstMessageFromWSServer) {
            bWaitingForFirstMessageFromWSServer = false;
    ...
    ws.onclose = function(event) {
        ...
        // In case the app is running under an iframe
        if (bIsInIframe) {
            window.top.postMessage("disconnected", "*");
        }
    };
}
